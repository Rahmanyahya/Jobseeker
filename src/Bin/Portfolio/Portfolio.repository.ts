...
export class PortfolioRepository {
    ...
    static async update(payload: Prisma.PortfolioUpdateInput, where: Prisma.PortfolioWhereUniqueInput): Promise<void> {
    ...
    static async findOne(where: Prisma.PortfolioWhereUniqueInput, userId: number): Promise<Omit<Portfolio, 'userId' | 'createdAt' | 'updatedAt'> | null> {
        return await prisma.portfolio.findUnique({ where: {
            id: where.id,
            userId
        }, omit: { userId: true, createdAt: true, updatedAt: true } });
    }

    static async findAll(paylod: GetAllPortfolio): Promise<{ data: Omit<Portfolio, 'userId' | 'createdAt' | 'updatedAt'>[], MetaData: MetaData }> {
